...
df_clean<- df %>% 
  ...
  mutate(
    ...
    Release.Clause = as.numeric(str_replace_all(str_remove_all(Release.Clause, "€"), c("K" = "000", "M" = "000000"))),
...
df_clean<-df_clean %>% mutate( 'Performance Category' = case_when(
  ...
  mutate(
    x_pos = case_when(
      `Performance Category` == "Emerging High Performers" ~ 90,
      `Performance Category` == "High Peak Performers" ~ 90,
      `Performance Category` == "Future High Performers" ~ 50,
      `Performance Category` == "Limited Performance" ~ 50
    ),
    y_pos = case_when(
      `Performance Category` == "Emerging High Performers" ~ 95,
      `Performance Category` == "High Peak Performers" ~ 55,
      `Performance Category` == "Future High Performers" ~ 95,
      `Performance Category` == "Limited Performance" ~ 55
    )
  ) %>%
...
### Amount of players across Top Football Leagues
...
The **Argentine Primera División** has **28 clubs**, which is more than the usual **20 clubs** found in most top leagues. This larger number of teams naturally leads to a higher total number of registered players. In contrast, the **Campeonato Brasileiro Série A** has **20 clubs**, similar to many major European leagues.
...
### Overall Score and Age Distribution Across Top Football Leagues
...
ggplot(df_clean%>% filter(!is.na(`Performance Category`))) +
  ...
  distinct(LeagueRanked, `Performance Category`, .keep_all = TRUE) ,
            ...
            inherit.aes = FALSE, size = 4.5, fontface = "bold",hjust = 0.5) +

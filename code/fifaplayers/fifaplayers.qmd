...
theme_ipsum_rc <-theme_ipsum_rc(base_family = "ibm_plex_sans")+
  theme(
    text=element_text(size=16, color="#333333"),
    axis.title.x = element_text(size = 16),
    axis.title.y = element_text(size = 16),
    plot.title = element_text(size=18, hjust = 0,vjust = 1,family = "fs", face = "bold", color="666666"),
    axis.text = element_text(size = 16,family = "ibm_plex_sans"),
    geom_text =element_text(size = 16,family = "ibm_plex_sans"),
...
```{r,  fig.width=20, fig.height=10, warning=FALSE, echo = FALSE, message=FALSE}
...
```{r, warning=FALSE, fig.width=20, fig.height=10, echo = FALSE, message=FALSE}

ggplot(df_clean %>% filter(!is.na(`Performance Category`))) + aes(x = Overall.Score, y = Potential.Score, color = `Performance Category`)+ geom_jitter(alpha = 0.6) + scale_color_manual(values = c( "Emerging High Performers" = "#91D1C2B2","High Peak Performers" = "gray20","Future High Performers" ="#1b9e77","Limited Performance" = "666666")) + geom_text(data = df_clean%>% group_by(LeagueRanked,`Performance Category`)%>% distinct(LeagueRanked, `Performance Category`, .keep_all = TRUE) , aes(x = x_pos, y = y_pos, label = paste0(Percentage, "%")), inherit.aes = FALSE, size = 4.5,hjust = 0.5) + facet_wrap(vars(reorder(LeagueRanked, League_Rank)), ncol = 5L) + labs(title = "Where Future Talants Emerge", x = "Overall Score (Mean = 70)", y = "Potential Score (Mean = 75)") + theme(legend.position = "right",legend.key.height = unit(1, "cm"), plot.title = element_text(size = 14))+ guides(color = guide_legend(override.aes = list(size = 8)))
...
```{r,  fig.width=11, fig.height=10, warning=FALSE, echo = FALSE , message=FALSE}
